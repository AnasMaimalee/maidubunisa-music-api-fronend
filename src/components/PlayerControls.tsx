// MainScreen.tsx - FULLY FIXED (No overlap + PlayerScreen navigation)
...
export default function MainScreen() {
  ...
  // 🔥 SONG PRESS - OPEN PLAYERSCREEN
  const handleSongPress = (song: Song) => {
    navigation.navigate('Player', { song }); // ✅ FULL PLAYER SCREEN
  };
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <SongCard
              song={item}
              onPress={() => handleSongPress(item)} // ✅ NAVIGATES TO PLAYERSCREEN
              rightAction={
                <FavoriteButton
                  songId={item.id}
      ...
      {/* 🔥 MINI PLAYER - PERFECT NO OVERLAP */}
      {currentSong && (
...
const styles = StyleSheet.create({
  ...
  scrollContent: {
    padding: 20,
    paddingBottom: 180, // ✅ FIXED - Perfect space for mini player
  },
  sectionTitle: {
    fontSize: 22,
    fontWeight: '700',
  ...
  // 🔥 PERFECT MINI PLAYER - NO OVERLAP
  playerContainer: {
    position: 'absolute',
    bottom: 0,
    left: 0,
    right: 0,
    height: 140,        // ✅ Fixed height with padding
    backgroundColor: '#fff',
    borderTopWidth: 1,
    borderTopColor: '#e0e0e0',
    paddingBottom: 30,  // ✅ Safe area for iPhone/Android
    paddingTop: 20,
    shadowColor: '#000',
